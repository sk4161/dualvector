...
class VAEStyleMultiRefCNN(nn.Module, DualChannel):
    def __init__(
        ...
        self.latent_encoder = (
            models.make(latent_encoder) if latent_encoder is not None else None
        )
        self.mlp_head = MLP(
            [z_dim, z_dim], bias=True, activate="gelu", activate_last=True
        )
    ...
    def init(self):
        ...
        if self.submodule_config is not None:
            for d in self.submodule_config:
                ...
                for key, name, freeze in zip(
                    d["ckpt_key"], d["module_name"], d["freeze"]
                ):
                    print(
                        "load", name, "from", ckpt_path + "." + key, "freeze:", freeze
                    )
    ...
    def ref_img_encode(self, refs, n_ref):
        if self.encode_type == "cnn":
            b, n = refs.shape[:2]

            imgs = rearrange(refs, "b n c h w -> (b n) c h w")
            z_imgs = self.img_encoder(imgs)
            z_imgs = rearrange(z_imgs, "(b n) z -> b n z", b=b)

            mask = torch.arange(n).to(imgs.device).unsqueeze(0).expand(
                b, -1
            ) >= n_ref.unsqueeze(1)
    ...
    def forward(self, batch, **kwargs):
        refs = batch["refs"]
        n_ref = batch["n_ref"]
        mu, log_var = self.ref_img_encode(refs, n_ref)
        ...
        if not self.train_latent or not is_train:
            ...
            if self.use_diffvg:
                res = self.render_paths_diffvg(
                    curves, sidelength=batch["img_origin_res"].size(-1)
                )
                out.update(res)
            else:
                res = self.render_paths_sdf(
                    curves, xy, use_occ=True, kernel=4 / self.sidelength
                )
                out.update(res)
